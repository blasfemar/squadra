...
<head>
    ...
    <title><?php echo isset($pageTitle) ? htmlspecialchars($pageTitle) : 'Admin'; ?> - Squadra SEO</title>
...
<body class="bg-gray-100">
    ...
    <aside class="sidebar bg-gray-900 text-gray-200 p-5 space-y-6 fixed top-0 left-0 h-full shadow-xl sidebar-custom-scroll overflow-y-auto">
        <div class="text-2xl font-bold text-white mb-6 border-b border-gray-700 pb-4">
            Squadra SEO
            <span class="block text-xs text-gray-400 mt-1">Admin v<?php echo htmlspecialchars($version); ?></span>
        </div>
        <nav class="flex-grow">
            <ul class="space-y-2">
                <li>
    ...
    <div class="content-wrapper ml-64"> <!-- ml-64 to offset sidebar width -->
        <header class="bg-white shadow p-4 page-header-sticky h-16 flex items-center"> <!-- h-16 for fixed height -->
            <h1 class="text-2xl font-semibold text-gray-800"><?php echo isset($pageTitle) ? htmlspecialchars($pageTitle) : 'Admin Area'; ?></h1>
        </header>
        ...
        <main class="p-6 bg-gray-100 flex-grow main-content-area">
            ...
            if (isset($pageContent)) {
                ...
                echo '<div class="bg-white p-6 rounded-lg shadow-md"><p class="text-red-500">Error: Page content was not provided to the layout.</p></div>';
            }
            ?>
        </main>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (updateAppLink) {
            updateAppLink.addEventListener('click', function(event) {
                ...
                if (confirm('Are you sure you want to attempt to update the application? This will run `git pull`. Make sure you have a backup if needed.')) {
